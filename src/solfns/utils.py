...
def equalize_arrays(
    ...
    return np.array(
        list(map(partial(pad_start, length=max_length, value=value), array_list))
    )


def pad_start(array: Union[List, np.ndarray], length: int, value=None) -> np.ndarray:
    """Pad 'array' with it's first value until it len(array) = length.
    ...
    if value is None:
        value = array_np[0]
    return np.concatenate([np.repeat([value], length - len(array_np)), array_np])
